...
def show_quarterly_screener():
    ...
    if st.session_state.screener_data is not None and len(st.session_state.screener_data) > 0:
        ...
        # First row: Company, Price, Market Cap
        ...
        with col2:
            st.markdown("💰 **Price (₹)**")
            price_min_default = float(df['Price'].min()) if 'Price' in df.columns and df['Price'].notna().any() else 0.0
            price_max_default = float(df['Price'].max()) if 'Price' in df.columns and df['Price'].notna().any() else 10000.0
            pcol1, pcol2 = st.columns(2)
            with pcol1:
                price_min_val = st.number_input("Min", value=price_min_default, key="screener_price_min")
            with pcol2:
                price_max_val = st.number_input("Max", value=price_max_default, key="screener_price_max")
            price_range = (price_min_val, price_max_val)
        
        with col3:
            st.markdown("📈 **Market Cap (Cr)**")
            mcap_min_default = float(df['Market_Cap'].min()) if 'Market_Cap' in df.columns and df['Market_Cap'].notna().any() else 0.0
            mcap_max_default = float(df['Market_Cap'].max()) if 'Market_Cap' in df.columns and df['Market_Cap'].notna().any() else 50000.0
            mcol1, mcol2 = st.columns(2)
            with mcol1:
                mcap_min_val = st.number_input("Min", value=mcap_min_default, key="screener_mcap_min")
            with mcol2:
                mcap_max_val = st.number_input("Max", value=mcap_max_default, key="screener_mcap_max")
            mcap_range = (mcap_min_val, mcap_max_val)
        
        # Second row: YOY filters
        ...
        with col5:
            ...
            with scol1:
                sales_min = st.number_input("Min", value=-100.0, key="screener_sales_min")
            with scol2:
                sales_max = st.number_input("Max", value=500.0, key="screener_sales_max")
            sales_yoy = (sales_min, sales_max)
        
        with col6:
            st.markdown("💹 **EBIDT YOY %**")
            ecol1, ecol2 = st.columns(2)
            with ecol1:
                ebidt_min = st.number_input("Min", value=-100.0, key="screener_ebidt_min")
            with ecol2:
                ebidt_max = st.number_input("Max", value=500.0, key="screener_ebidt_max")
            ebidt_yoy = (ebidt_min, ebidt_max)
        
        with col7:
            st.markdown("💵 **Net Profit YOY %**")
            pcol1, pcol2 = st.columns(2)
            with pcol1:
                profit_min = st.number_input("Min", value=-100.0, key="screener_profit_min")
            with pcol2:
                profit_max = st.number_input("Max", value=500.0, key="screener_profit_max")
            profit_yoy = (profit_min, profit_max)
        
        with col8:
            st.markdown("📉 **EPS YOY %**")
            epcol1, epcol2 = st.columns(2)
            with epcol1:
                eps_min = st.number_input("Min", value=-100.0, key="screener_eps_min")
            with epcol2:
                eps_max = st.number_input("Max", value=500.0, key="screener_eps_max")
            eps_yoy = (eps_min, eps_max)
